...
class SubInstructionTest {
  ...
  void execute2() {
    ...
    instr.execute(m);
    assert m.getRegisters().getRegister(0) == -4;
  }
  ...
  void execute4() {
    ...
    instr.execute(m);
    assert m.getRegisters().getRegister(6) == -200;
  }
  ...
  void testToString1() {
    instr = new SubInstruction("L0", 0, 0, 0);
    assert instr.toString().equals("L0: sub. Store result of r0 - r0 in r0");
  }

  @Test
  void testToString2() {
    instr = new SubInstruction("DEF", 6, 10, 18);
    assert instr.toString().equals("DEF: sub. Store result of r10 - r18 in r6");
  }
}